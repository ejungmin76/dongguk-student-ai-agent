...
university_knowledge가 필요하면 search_query에 검색에 유용한 짧은 질의를 작성하라.
원문에 있는 표현과 문맥상 명확한 과목명·학수번호·공식 용어만 정규화하고, 새로운
사실이나 답을 추론하지 마라. 확신이 없으면 null로 둔다. 다른 기능만 필요하면 null이다.
