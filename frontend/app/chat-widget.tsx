...
export default function ChatWidget({ autoOpen = false, extensionMode = false }: { autoOpen?: boolean; extensionMode?: boolean }) {
  ...
  useEffect(() => {
    const receive = (event: MessageEvent) => {
      if (event.data?.type === "DGU_NDRIMS_ACTION_RESULT") {
        const message = event.data.reason || (event.data.opened ? "nDRIMS 메뉴를 열었습니다." : "현재 화면에서 메뉴를 찾지 못했습니다.");
        setMenuStatus(message);
        if (!event.data.opened) setError(message);
      }
    };
