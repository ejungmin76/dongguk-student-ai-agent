...
확장프로그램 전용 페이지가 로컬 챗봇을 불러오므로 nDRIMS의 로컬 네트워크 차단을 피합니다. 답변에 검증된 nDRIMS Action이 오면 확장프로그램이 경로의 마지막 메뉴를 현재 화면에서 찾아 클릭합니다. 메뉴 DOM이 바뀐 경우에는 경로만 안내하고 자동 클릭하지 않습니다.
